...
def main():
    ...
    parser.add_argument('--fw',
                        ...
                        default='1100')
    parser.add_argument(f'--stage1', default='stage1/stage1')
    parser.add_argument('--stage2', default='stage2/stage2')
    args = parser.parse_args()

    print('[+] PPPwn - PlayStation 4 PPPoE RCE by theflow')
    print('[+] args: ' + ' '.join(f'{k}={v}' for k, v in vars(args).items()))

    with open(args.stage1 + "-" + args.fw + ".bin", mode='rb') as f:
        stage1 = f.read()

    with open(args.stage2 + "-" + args.fw + ".bin", mode='rb') as f:
        stage2 = f.read()
